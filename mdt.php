...
class MdtDashboardModule extends MdtModule
{
    ...
    protected function displayAction()
    {
        ...
        $actionsHtml = '
            ...
            [ <a href="' . MdtApp::getUrl('dashboard/addcustomer') . '">add customer</a> ]<br />
    ...
    protected function tplhintsAction() {
        $request = MdtApp::getRequest();
        ...
        switch ($operation)
        {
            case 'enable':
            {
                $this->setTplhintsEnabled();
                MdtApp::success('template hints enabled');
                break;
            }

            case 'disable':
            {
                $this->setTplhintsEnabled(false);
                MdtApp::success('template hints  disabled');
                break;
            }
    ...
    private function getTplhintsEnabled()
    {
        return (bool)MdtDB::getField('value', 'core_config_data', 'path = "dev/debug/template_hints"');
    }

    private function setTplhintsEnabled($enabled = true)
    {
        MdtDB::update(array('value' => (int)$enabled),  'core_config_data', 'path = "dev/debug/template_hints"');
        MdtDB::update(array('value' => (int)$enabled),  'core_config_data', 'path = "dev/debug/template_hints_blocks"');
    }
    ...
    private function getModules()
    {
        ...
        foreach (glob('app/etc/modules/*.xml') as $file)
        {
...
class MdtReportsModule extends MdtModule
{
    ...
    protected function archiveAction()
    {
        if (!file_exists('var/log/' . $this->filename . '.log'))
        {
            MdtApp::warning('var/log/' . $this->filename . '.log not found');
        }
        else
        {
            if (rename('var/log/' . $this->filename . '.log', 'var/log/' . $this->filename . '.log.archive'))
            {
                MdtApp::success($this->filename . '.log archived');
            }
            else
            {
                MdtApp::error('var/log/' . $this->filename . '.log could not be renamed', false);
            }
        }
...
class MdtXmlModule extends MdtXml
{
    ...
    public function __construct($moduleName)
    {
        ...
        parent::__construct('app/etc/modules/' . $this->moduleName . '.xml');
    }
...
class MdtXmlLocal extends MdtXml
{
    ...
    {
        parent::__construct('app/etc/local.xml');
    }
...
<head>
<link href="data:image/x-icon;base64,iVBORw0KGgoAAAANSUhEUgAAAAoAAAAKCAYAAACNMs+9AAAAOElEQVQYV2NkIBIwEqmOgSSF/xkYwBpgNMgSZDGwpTAFMBcg82G2gTUhm4RuCoZCovxDkmeoayIAz7kLCkZeYPIAAAAASUVORK5CYII=" rel="icon" type="image/x-icon" />
